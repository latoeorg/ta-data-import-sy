...
class OEESummaryController extends Controller
{
    public function index()
    {
        ...
        foreach ($items as $item) {
            ...
            $item->Available_Rate = number_format((($item->Operating_Time / $item->Available_T) * 100) / 100, 0);
            ...
            $item->Performance_Rate = number_format((($item->PlanCycleTime / $item->ActualCycleTime) * 100) / 100, 0);

            // Quality
            $item->Reject_Total = $dataPerMonth->sum('Reject_Total');
            $item->Finish_Total = $item->Output - $item->Reject_Total;
            $item->Quality_Rate = number_format($item->Output / ($item->Output + $item->Reject_Total), 0);

            // OEE
            $item->OEE = number_format($item->Available_Rate * $item->Performance_Rate * $item->Quality_Rate, 0);
        }
